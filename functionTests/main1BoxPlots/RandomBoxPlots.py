...
'''
W = skfda.misc.inner_product_matrix(datam['data'].basis, datam['data'].basis)
W[W < 1.e-15] = 0
...
data = []
mats = {'W': [], 'W_m': []}
with open('../main1Tests/W.csv', newline='') as csvfile:
...
while(j < itTest):
    ...
    res = tfun._T_funhddc_main1(fd, Wlist, K, df_start, 'numeric', 'yes', 'AKJBKQKDK', itermax, threshold, 'cattell', 1.e-6, 'random', None, None, 2, noise, None, None, d_max, d_set, None)
...
filename = 'RandomNumericCattellYes.csv'
...
while j < itTest:
    print(j)
    startTime = time.process_time()
    res = tfun._T_funhddc_main1(fd, Wlist, K, df_start, 'numeric', 'yes', 'AKJBKQKDK', itermax, threshold, 'cattell', 1.e-6, 'random', None, None, 2, noise, None, None, d_max, d_set, known)
    if not isinstance(res, tfun.TFunHDDC):
        continue
    print(res.cl)
...
filename = 'RandomNumericCattellYesKnown.csv'
with open(filename, 'w', newline='') as csvfile:
    datawriter = csv.writer(csvfile)
...
axs[0,0].set_title('BIC for Random init without Known')

axs[0, 1].boxplot(stats1['icl'])
axs[0,1].set_title('ICL for Random init without Known')

axs[1,0].boxplot(stats2['bic'])
axs[1,0].set_title('BIC for Random init with Known')

axs[1,1].boxplot(stats2['icl'])
axs[1,1].set_title('ICL for Random init with Known')
