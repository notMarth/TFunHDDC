...
W = inprod(data$fd$basis, data$fd$basis)
W[W<1e-15] = 0
W_m = chol(W)
dety = det(W)
...
df_start=50
itermax =200
known = rep(NA, length(data$groupd))
known[1:length(data$groupd) %% 3 - 1 == 0] = data$groupd[1:length(data$groupd) %% 3 - 1 == 0]
itTest = 50
stats1 = list(bic = c(), icl = c(), time=0, cls = list('1'=rep(0,length(data$groupd)), '2'=rep(0,length(data$groupd)), '3'=rep(0,length(data$groupd))), conv = 0)
stats2 = stats1
j = 1
...
while (j <= itTest){
  print(j)
  startTime = Sys.time()
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='mini-em', init.vector=NULL, mini.nb=c(5,10), min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=NULL)
  if (!inherits(res, 'tfunHDDC')){
    next
  }
  print(res$class)
  ...
  for(i in 1:length(data$groupd)){
    stats1$cls[[as.character(res$class[i])]][i] = stats1$cls[[as.character(res$class[i])]][i] + 1
  }
  
  if(res$converged){
    stats1$conv = stats1$conv + 1
  }
  
  stats1$time = stats1$time + (Sys.time() - startTime)
  
  j = j + 1
}

stats1$time = as.numeric((stats1$time)/itTest)

filename = 'RMiniNumericCattellYes.csv'
write.csv(list(time=stats1$time, cl=stats1$cl, conv=stats1$conv), filename)
...
while (j <= itTest){
  print(j)
  startTime = Sys.time()
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='mini-em', init.vector=NULL, mini.nb=c(5,10), min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=known)
  if (!inherits(res, 'tfunHDDC')){
    next
  }
  print(res$class)
  ...
  for(i in 1:length(data$groupd)){
    stats2$cls[[as.character(res$class[i])]][i] = stats2$cls[[as.character(res$class[i])]][i] + 1
  }
  
  if(res$converged){
    stats2$conv = stats2$conv + 1
  }
  
  stats2$time = stats2$time + (Sys.time() - startTime)
  
  
  j = j + 1
}

stats2$time = as.numeric((stats2$time)/itTest)

filename = 'RMiniNumericCattellYesKnown.csv'
write.csv(list(time=stats2$time, cl=stats2$cl, conv=stats2$conv), filename)


dataList = list(BIC=stats1$bic, ICL=stats1$icl, BICwKnown=stats2$bic, ICLwKnown=stats2$icl)
boxplot(x=dataList, main='Mini-em Init')