...
while (j <= itTest){
  print(j)
  startTime = Sys.time()
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='random', init.vector=NULL, mini.nb=NULL, min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=NULL)
...
filename = 'RRandomNumericCattellYes.csv'
write.csv(list(time=stats1$time, cls=stats1$cls, conv=stats1$conv), filename)
...
while (j <= itTest){
  print(j)
  startTime = Sys.time()
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='random', init.vector=NULL, mini.nb=NULL, min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=known)
  if (!inherits(res, 'tfunHDDC')){
    next
  }
  print(res$class)
...
filename = 'RRandomNumericCattellYesKnown.csv'
write.csv(list(time=stats2$time, cls=stats2$cls, conv=stats2$conv), filename)

dataList = list(BIC=stats1$bic, ICL=stats1$icl, BICwKnown=stats2$bic, ICLwKnown=stats2$icl)
boxplot(x=dataList, main='Random Init')
