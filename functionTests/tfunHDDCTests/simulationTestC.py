...
if __name__ == '__main__':
    ...
    for i in range(itTest):    
        ...
        if i == 0:
             trainingdata.plot(group = labels, group_colors=['red', 'blue', 'green'], linestyle='--')
             plt.show()
        ...
        for j in inits:
            print(i, j)
            res = tfun.tfunHDDC(trainingdata, K=K, threshold=thresh, model=models, init=j, init_vector=vec, nb_rep=50, min_individuals=2, mc_cores=coreNum)
            if isinstance(res, tfun.TFunHDDC):
                ...
                print(tfun._T_hddc_ari(res.cl, labels))
                    ...
                    results[f'{i}_{j}_{res.model}_{res.threshold}'] = [f'{i}_{j}_{res.model}_{res.threshold}', 'NA', 'NA']
