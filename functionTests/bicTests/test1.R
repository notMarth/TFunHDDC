...
params = list(model='AKJBKQKDK', a=c(1), b=c(0,1), d=c(1,2,3), N=3, loglik=5, posterior=matrix(c(1,2,2,2,2,2,2,2,2), 3, 3), K=3, prop=c(0))
print(.T_hdclassift_bic(params, 3, 'no'))
...
vec = rep(3, length(data$groupd))
vec[1:50] = 1
vec[51:100] =2
itTest = 100
stats1 = list(bic = 0, icl = 0, cls = list('1'=rep(0,length(data$groupd)), '2'=rep(0,length(data$groupd)), '3'=rep(0,length(data$groupd))), conv = 0)
stats2 = stats1
j = 1

while (j <= 1){
  print(j)
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='vector', init.vector=vec, mini.nb=NULL, min.individuals=0, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=NULL)
  if (!inherits(res, 'tfunHDDC')){
    next
  }
  print(res$class)
  stats1$bic = stats1$bic + res$BIC
  stats1$icl = stats1$icl + res$ICL
...
vec[1:length(data$groupd) %% 3 - 1 == 0] = known[1:length(data$groupd) %% 3 - 1 == 0]
...
while (j <= 1){
  print(j)
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='vector', init.vector=vec, mini.nb=NULL, min.individuals=0, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=known)
  if (!inherits(res, 'tfunHDDC')){
    next
  }
  print(res$class)
  stats2$bic = stats2$bic + res$BIC
  stats2$icl = stats2$icl + res$ICL
