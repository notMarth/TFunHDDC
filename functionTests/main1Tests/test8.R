...
res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='kmeans', init.vector=NULL, mini.nb=c(5,10), min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=known)
print(res$converged)
print(res$BIC)
print(res$ICL)
print(res$class)