...
while (j <= 100){
  print(j)
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='vector', init.vector=vec, mini.nb=NULL, min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=NULL)
...
filename = 'RVectorNumericCattellYes.csv'
write.csv(stats1, filename)

j = 1

while (j <= 100){
  print(j)
  res = .T_funhddc_main1(fdobj = data$fd, Wlist=W_list, K=K, dfstart=df_start, dfupdate='numeric', dfconstr = 'yes', model='AKJBKQKDK', itermax=itermax, threshold=threshold, method='cattell', eps=1e-6, init='vector', init.vector=vec, mini.nb=NULL, min.individuals=2, noise.ctrl=noise, com_dim=NULL, kmeans.control=kmc, d_max=D_max, d_set=d_set, known=known)
  if (!inherits(res, 'tfunHDDC')){
    next
  }
  print(res$class)
...
filename = 'RVectorNumericCattellYesKnown.csv'
write.csv(stats2, filename)