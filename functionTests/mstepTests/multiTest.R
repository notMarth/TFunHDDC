  ...
  for(method in methods){
    for(constr in dfconstr){
      for(update in dfupdate){
        ...
        res = TFunHDDC:::.T_funhddt_init(data, Wlist, K, t, nux, model, threshold, method, noise, NULL, d_max, d_set)
        tw = TFunHDDC:::.T_funhddt_twinit(data, Wlist, res, nux)
        res = TFunHDDC:::.T_funhddt_m_step1(data, Wlist, K, t, tw, nux, update, constr, model, threshold, method, noise, NULL, d_max, d_set)
      }
    }
  }
}