...
models = ["AKJBKQKDK", 'AKBKQKDK', 'ABKQKDK', 'ABQKDK']
...
for i in models:
    for j in method:
        ...
        initx = _T_funhddt_init(data, Wlist, K, t, nux, i, threshold, j, noise, None, d_max, d_set)
        
        tw = _T_funhddt_twinit(data, Wlist, initx, nux)

        for k in dfupdate:
            for q in dfconstr:
                m = _T_funhddt_m_step1(data, Wlist, K, t, tw, nux, k, q, i, threshold, j, noise, None, d_max, d_set)
                temp = {'a':m['a'], 'b':m['b'], 'd':m['d'], 'nux':m['nux'], 'prop':m['prop'], 'mu':m['mu'], 'Q':m['Q'], 'Q1':m['Q1'], 'model': i, 'method':j, 'dfupdate': k, 'dfconstr':q}
                filename = 'QPy' + i + j + k + q + '.csv'

                #for z in range(len(m['Q1'])):
                    #print(m['Q1'][f'{z}'].shape)
                e = _T_funhddt_e_step1(data, Wlist, m, 0, None, None)
                with open(filename, 'w', newline='') as csvfile:
                    datawriter = csv.writer(csvfile)
                    for r in e.items():
                        if r[0] != 'L':
                            for z in r[1]:
                                datawriter.writerow(z)
